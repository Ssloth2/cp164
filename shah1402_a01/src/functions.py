...
def clean_list(values):
    ...
    # Use a new list to store the unique values in order
    unique_values = []

    for value in values:
        # If the value is not in the set, it's unique
        if value not in seen_values:
            unique_values.append(value)
            seen_values.add(value)

    # Update the original list with the unique values
...
def list_subtraction(minuend, subtrahend):
    ...
    ------------------------------------------------------
    """
    minuend = [element for element in minuend if element not in subtrahend]

    print(f"Output: {minuend}")
    return
...
def max_diff(a):
    ...
    """
    if len(a) < 2:
        # If the list has less than two elements, there are no adjacent values.
        # You may choose to handle this case based on your requirements.
        return None

    # Use the max function with a key function to find the maximum absolute difference.
    md = max(abs(a[i] - a[i + 1]) for i in range(len(a) - 1))
