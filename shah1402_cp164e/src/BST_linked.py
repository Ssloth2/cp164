...
class BST:
    ...
    def _are_flipped_aux(self, node1, node2):
        """
        -------------------------------------------------------
        ...
        if node1 is None and node2 is None:
            flipped = True
        # If only one node is None, they are not flipped versions of each other
        elif node1 is None or node2 is None:
            flipped = False
        elif node1 is not None and node2 is not None:
            flipped = (self._are_flipped_aux(node1._left, node2._right) and
                       self._are_flipped_aux(node1._right, node2._left))
    ...
    # C
        if (node is None):
            ...
            lHeight = self._total_height_aux(node._left)+1
            rHeight = self._total_height_aux(node._right)+1
